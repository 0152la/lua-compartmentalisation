...
comp_from_elf(char* filename)
{
    ...
    for (size_t i = 0; i < comp_ehdr.e_shnum; ++i)
    {
        ...
        if (comp_symtb_hdr.sh_type == SHT_SYMTAB)
        {
            ...
            /*pread((int) new_comp->fd, &comp_symtb, comp_symtb_hdr.sh_size, comp_symtb_hdr.sh_offset);*/
            ...
            for (size_t j = 0; j < syms_count; ++j)
            {
                ...
                /*pread_res = pread((int) new_comp->fd, &curr_sym, sizeof(Elf64_Sym), comp_ehdr.e_shoff + comp_symtb_hdr.sh_offset + j * sizeof(Elf64_Sym));*/
                ...
                /*else if(!strcmp("__vdso_clock_gettime", &comp_strtb[curr_sym.st_name]))*/
                /*{*/
                    /*printf("Found `__vdso_clock_gettime` func at %p.\n", (void*) curr_sym.st_value);*/
                    /*uintptr_t init_addr = curr_sym.st_value;*/
                    /*uintptr_t to_branch_addr = (uintptr_t) manager___vdso_clock_gettime;*/
                    /*int offset = (to_branch_addr - init_addr) / 4;*/
                    /*assert(offset < (1 << 27));*/
                    /*offset &= (1 << 26) - 1;*/
                    /*const int amd_b_instr_mask = 0b101 << 26;*/
                    /*uintptr_t instr_binary = amd_b_instr_mask | offset;*/
                    /*int* init_addr_ptr = (void*) init_addr;*/
                    /*struct vsdo_patch new_patch = { init_addr_ptr, instr_binary };*/
                    /*new_comp->patches[0] = new_patch; // TODO*/
                    /*found += 1;*/
                /*}*/
                if (found == to_find)
                {
